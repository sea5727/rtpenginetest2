...
destroy:
#if PROJECT_KTP_PTTGW
		 if(jdin_tun.callback_port){
			 if(nEventSocket < 0)
			 	nEventSocket = socket(AF_INET, SOCK_DGRAM, 0);
			 if(nEventSocket > 0){
				memset(&servaddr, 0, sizeof(servaddr));
                servaddr.sin_family = AF_INET;
                servaddr.sin_addr.s_addr = inet_addr(inet_ntoa(ca->created_from_addr.u.ipv4));
                servaddr.sin_port = htons(jdin_tun.callback_port);

				strncpy(szBuf, ca->callid.s, ca->callid.len);
                szBuf[ca->callid.len] = 0x00;

                if (nEventSocket > 0 && sendto(nEventSocket, szBuf, sizeof(szBuf), 0, (struct sockaddr *)&servaddr, sizeof(servaddr)) > 0)
                    ilog(LOG_NOTICE, "%s send rtp timeout message to %s:%d", szBuf, inet_ntoa(ca->created_from_addr.u.ipv4), jdin_tun.callback_port);
                else
                    ilog(LOG_NOTICE, "%s send fail rtp timeout message %s", szBuf, strerror(errno));
			 }
		 }
#endif
		call_destroy(ca);
